...
public class JavaClientCodegen extends AbstractJavaCodegen
    ...
    public JavaClientCodegen() {
        ...
        cliOptions.add(CliOption.newBoolean(USE_RECORDS, "Render the implementations of a generated oneOf interface (useOneOfInterfaces=true) as Java records instead of classes. Only supported for the microprofile library with serializationLibrary=jackson.", this.useRecords));
    ...
    public void processOpts() {
        ...
        if (libMicroprofile && (useSealedOneOfInterfaces || useRecords)) {
            writePropertyBack(JAVA_17, true);
        }
...
//            this.setMicroprofileFramework(additionalProperties.get(MICROPROFILE_FRAMEWORK).toString());
    ...
    public Map<String, ModelsMap> postProcessAllModels(Map<String, ModelsMap> objs) {
        ...
        // Collect the classnames of the interfaces generated by the useOneOfInterfaces feature so that
        // we can identify which concrete models implement one of them (and are therefore candidates to
        // be rendered as records).
        ...
        for (ModelsMap modelsMap : result.values()) {
            for (ModelMap modelMap : modelsMap.getModels()) {
                CodegenModel model = modelMap.getModel();
                if (isOneOfInterfaceRecordCandidate(model, oneOfInterfaceNames)) {
                    model.getVendorExtensions().put("x-java-record", true);
                }
            }
        }
        return result;
